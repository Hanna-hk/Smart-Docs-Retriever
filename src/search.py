"""
Searches the best answer to the request and saves it into the database
"""
import numpy
...
class Searcher:
    def __init__(self, data_file, numpy_file, request):
        ...
        # Loading the original data chunks and the dense vector embeddings of the chunks
    ...
    def request_processing(self, threshold=0.25):
        ...
        # Perform cosine similarity search between the query vector and the document matrix 
        # to find the top 3 most relevant matches.
        self.cosine = util.semantic_search(encoded_req, self.data_numpy, top_k=3)[0]
        logging.info("Cosine Similarity has been calculated")

        # Load all the answers with high score to the list
        answers=[]
        ...
        if answers==[]:
            ...
            # Loading the information about the request to the database
            best_ans = answers[0]
...
if __name__=="__main__":
    # Compliting all processes: from data loading to searching
    try:
        ...
        # Processing the data if haven't been already processed
